...
> PalPeek 只提供观战，不提供远程控制。当前版本为 **1.0.4**。主播端仅支持 **Windows 11 x64**；网页观众支持现代 Windows、macOS、Linux、iOS 和 Android 浏览器。
...
- 网页观战会等待首个 HLS 分片真正就绪后再启动播放器，避免首次点击在媒体流尚未建立时触发密集重试。
- 播放失败或重新开始观战时会完整销毁旧的 hls.js 实例并释放旧租约，不再残留后台清单轮询。
- HLS 清单请求不再与控制接口共用全局限流额度，媒体请求不会再导致心跳收到 HTTP 429 并误报“连接已经过期”。
- 心跳遇到短暂网络故障时会继续重试，只有登录或租约确实失效时才要求重新开始观战。
- HLS 目标时长会根据实际分片时长生成，避免异常分片导致浏览器以错误频率刷新播放清单。

## 1.0.3 修复

- 修复 `setTarget` IPC 命令将 `sessionId` 错误序列化为 `SessionId`，导致内置 Host 保存空会话并拒绝启动网页媒体流的问题。
- 所有 PalPeek Host IPC 命令统一使用 camelCase 字段名，避免 C# 推断属性名与本机协议不一致。
- 设置捕获目标后会回读并验证 PID、窗口句柄和会话 ID；Host 未接受目标时不再错误显示“直播已就绪”。

## 1.0.2 修复

- 修复点击“开始观战”后，内置 Host 因目标状态短暂漂移而返回 `stale_session` 和 HTTP 503 的问题。
- 网页媒体流启动前会重新同步当前游戏窗口与会话，并对瞬时过期状态执行一次受控恢复。
- 已启动的网页流会与 Host 真实状态再次校验，Host 恢复后不会误用旧会话状态。

## 1.0.1 修复

- 网页音频改用 Windows 11 内置的 Media Foundation AAC-LC 编码器，不再依赖随包 FFmpeg 中未启用的 AAC 编码器。
- 修复点击“开始观战”后 `/api/web/v1/viewers` 因 AAC 编码器不可用而返回 HTTP 500 的问题。
- 网页媒体流启动失败时会释放已预约的观众名额，并返回可读的 503 错误提示。
- 发布流程会校验 PalPeek Host 源码状态和二进制提交版本，避免将过期或未提交源码构建的 Host 装入安装包。

## 1.0.0 新功能

- 网页观战总开关现在会按真实状态显示“已开启”或“已关闭”，避免误把已配置 Funnel 当成已开放观战。
- 总开关关闭时，观众会看到“主播尚未开启网页观战”的明确提示，不再误报“链接或口令无效”。
- 网页首次打开时使用无错误的会话探测，不再产生预期内的 401 控制台报错。
- 移除网页首屏内联样式，在保持深色首屏的同时满足严格的内容安全策略。

## 0.5.3 新功能

- 修复首次配置 Tailscale Funnel 时 PalPeek 只在后台等待、浏览器没有反应并最终超时的问题。
- PalPeek 现在会实时读取 Tailscale 的授权提示，并自动使用系统默认浏览器打开官方授权页面。
- 授权过程中会显示“打开授权页面”备用按钮；自动打开失败或浏览器窗口被遮挡时可以手动重开。
- 只允许自动打开 `https://login.tailscale.com` 的 HTTPS 地址，不会打开命令输出中的其他链接。
- 首次授权等待时间由 2 分钟调整为 5 分钟；取消配置时仍会终止对应的 Tailscale 进程及其子进程。

## 0.5.2 新功能

- “网页观战”在侧边栏中与其他入口保持同级，并增加“开发中”标记。
- 浏览器画质下拉框使用 PalPeek 自己的深色模板，选中项和下拉列表不再受 Windows 浅色主题影响。
- 配置 Funnel 前会先检查 Tailscale 是否已连接及 MagicDNS 是否可用；Tailscale 停止、未登录或返回 `NoState` 时显示可操作的简体中文提示，不再直接显示 CLI 内部错误。
- Funnel 配置使用官方 `--yes` 非交互模式，避免后台进程等待终端确认。

## 0.5.1 新功能

- “网页观战”入口移到主窗口侧边栏，并位于“观战大厅”正下方；主播可以直接在主窗口管理公网入口、画质、链接和口令，不再打开独立管理窗口。
- 主播端网页观战面板与观众网页统一使用 PalPeek 深色视觉体系；网页首屏、表单自动填充和视频等待界面均保持深色，不再出现白底闪烁。
...
1. 确认 Tailscale 已登录，然后在 PalPeek 主窗口侧边栏点击“网页观战”。
2. 在内嵌管理面板中开启“允许公网浏览器观战”。首次开启会显示公网暴露提醒。
3. 首次配置时，PalPeek 会自动打开 Tailscale 官方授权页面。按页面提示允许 Funnel，然后返回 PalPeek 等待配置完成；如果浏览器没有出现，点击面板底部的“打开授权页面”。
...
- **网页观战**：从主窗口侧边栏进入，管理公网入口、网页画质、固定邀请链接和口令。网页画质与 Moonlight 画质分别设置。
...
- 若提示“Tailscale 当前已停止”，请先打开 Tailscale 客户端并确认状态为“已连接”，再返回 PalPeek 配置入口。
- 首次配置入口时应自动打开 Tailscale 授权页；若没有出现，请点击面板底部的“打开授权页面”，完成授权后再次点击“配置入口”。
- 在主窗口侧边栏打开“网页观战”，确认总开关和 Funnel 公网入口均已开启。
