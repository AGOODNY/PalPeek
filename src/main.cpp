...
#include "video.h"
#ifdef _WIN32
  #include "platform/windows/palpeek_ipc.h"
#endif
...
#pragma GCC diagnostic pop
...
#ifdef _WIN32
  // PalPeek is a view-only product. These are hard-disabled in the host fork
  // so a changed config file or Moonlight client can never enable control.
  ...
  config::video.dd.resolution_option = config::video_t::dd_t::resolution_option_e::disabled;
  config::video.dd.refresh_rate_option = config::video_t::dd_t::refresh_rate_option_e::disabled;
  config::video.dd.hdr_option = config::video_t::dd_t::hdr_option_e::disabled;
  config::audio.install_steam_drivers = false;
#endif
  ...
  // the only capture source that this fork permits.
  std::thread palpeekControlThread {palpeek::start_control_pipe()};
  auto palpeek_control_thread_guard = util::fail_guard([&]() {
    palpeek::stop_control_pipe();
    if (palpeekControlThread.joinable()) {
      palpeekControlThread.join();
    }
  });

  if (palpeek::wait_for_capture_target(5s)) {
    palpeek::set_encoding_state(palpeek::encoding_state_t::probing);
    if (video::probe_encoders()) {
      palpeek::set_encoding_state(
        palpeek::encoding_state_t::error,
        "encoder_unavailable",
        "Sunshine could not initialize a video encoder for the selected game window"
      );
      BOOST_LOG(error) << "Video failed to find working encoder"sv;
    } else {
      palpeek::set_encoding_state(palpeek::encoding_state_t::ready);
    }
  } else {
    BOOST_LOG(info) << "PalPeek is waiting for a game capture target before probing encoders"sv;
  }
#else
  if (video::probe_encoders()) {
    BOOST_LOG(error) << "Video failed to find working encoder"sv;
