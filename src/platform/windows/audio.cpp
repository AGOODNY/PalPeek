...
#include "palpeek_audio_activation.h"
...
namespace platf::audio {
  ...
  using async_operation_t = util::safe_ptr<IActivateAudioInterfaceAsyncOperation, Release<IActivateAudioInterfaceAsyncOperation>>;

  class process_audio_activation_t: public IActivateAudioInterfaceCompletionHandler {
  public:
    process_audio_activation_t():
        completed(CreateEventA(nullptr, TRUE, FALSE, nullptr)) {
    }

    virtual ~process_audio_activation_t() = default;
    ...
    HRESULT STDMETHODCALLTYPE ActivateCompleted(IActivateAudioInterfaceAsyncOperation *operation) override {
      ...
      IUnknown *interface_pointer = nullptr;
      const auto operation_result = operation->GetActivateResult(&interface_result, &interface_pointer);
      activate_result = FAILED(operation_result) ? operation_result : interface_result;
      activated_interface = interface_pointer;
    ...
    HRESULT activate_result = E_PENDING;
  ...
  audio_client_t make_process_audio_client(DWORD root_pid, const format_t &format) {
    ...
    activation_params.ProcessLoopbackParams.TargetProcessId = root_pid;
    ...
    auto completion = new process_audio_activation_t();
    async_operation_t operation;
    auto status = ActivateAudioInterfaceAsync(
    ...
    if (FAILED(status)) {
      completion->Release();
      BOOST_LOG(error) << "PalPeek couldn't begin process audio activation [0x"
                       << util::hex(status).to_string_view() << ']';
      return nullptr;
    }

    if (WaitForSingleObject(completion->completed.get(), 5000) != WAIT_OBJECT_0 ||
        FAILED(completion->activate_result) ||
        !completion->activated_interface) {
      BOOST_LOG(error) << "PalPeek couldn't activate process-tree audio [0x"
                       << util::hex(completion->activate_result).to_string_view() << ']';
      // On timeout the async operation may still own this handler. Releasing
      // our reference keeps the callback safe and lets COM delete it later.
      completion->Release();
      return nullptr;
    }

    audio_client_t audio_client;
    status = completion->activated_interface->QueryInterface(IID_IAudioClient, (void **) &audio_client);
    completion->activated_interface->Release();
    completion->activated_interface = nullptr;
    completion->Release();
    if (FAILED(status)) {
      BOOST_LOG(error) << "PalPeek process audio did not expose IAudioClient [0x"
                       << util::hex(status).to_string_view() << ']';
      return nullptr;
    }
    ...
    status = audio_client->Initialize(
      ...
      0,
      0,
      reinterpret_cast<LPWAVEFORMATEX>(&capture_waveformat),
      nullptr
    );
    if (FAILED(status)) {
      BOOST_LOG(error) << "PalPeek couldn't initialize process-tree audio for ["
                       << format.name << "]: [0x" << util::hex(status).to_string_view() << ']';
      return nullptr;
    }

    BOOST_LOG(info) << "PalPeek capturing audio from process tree rooted at PID " << root_pid;
    return audio_client;
  }
  ...
  public:
    ...
    int init(std::uint32_t sample_rate, std::uint32_t frame_size, std::uint32_t channels_out, bool continuous) {
      ...
      const auto palpeek_target = palpeek::capture_target();
      if (!palpeek_target) {
        palpeek::set_audio_state(
          palpeek::audio_state_t::error,
          "audio_target_unavailable",
          "No authorized game process is available for audio capture"
        );
        BOOST_LOG(error) << "PalPeek refused audio capture because no game process target is set";
        return -1;
      }
      ...
      for (const auto &format : formats) {
        ...
        audio_client = make_process_audio_client(palpeek_target->root_pid, format);
      ...
      if (!audio_client) {
        palpeek::set_audio_state(
          palpeek::audio_state_t::error,
          "process_audio_unavailable",
          "Windows could not capture audio from the selected game process tree"
        );
      ...
      if (FAILED(status)) {
        palpeek::set_audio_state(
          ...
          "Windows could not query the process audio buffer"
        );
      ...
      if (FAILED(status)) {
        palpeek::set_audio_state(
          palpeek::audio_state_t::error,
          "process_audio_init_failed",
          "Windows did not provide a process audio capture service"
        );
      ...
      if (FAILED(status)) {
        palpeek::set_audio_state(
          palpeek::audio_state_t::error,
          "process_audio_init_failed",
          "Windows could not configure the process audio capture event"
        );
      ...
      palpeek::set_audio_state(palpeek::audio_state_t::ready);
      status = audio_client->Start();
      if (FAILED(status)) {
        palpeek::set_audio_state(
          palpeek::audio_state_t::error,
          "process_audio_start_failed",
          "Windows could not start process-scoped audio capture"
        );
      ...
      palpeek::set_audio_state(palpeek::audio_state_t::capturing);
      return 0;
    }
    ...
    ~mic_wasapi_t() override {
      ...
      if (capture_started) {
        palpeek::set_audio_state(palpeek::audio_state_t::idle);
      }
      ...
      switch (status) {
        ...
        default:
          palpeek::set_audio_state(
            ...
            "Windows process audio capture event failed"
          );
