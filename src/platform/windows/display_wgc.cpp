...
namespace platf::dxgi {
  ...
  int wgc_capture_t::init(display_base_t *display, const ::video::config_t &config) {
    ...
    if (monitor_factory == nullptr) {
      palpeek::set_capture_state(
        palpeek::capture_state_t::error,
        "capture_api_unavailable",
        "Windows Graphics Capture activation factory is unavailable"
      );
      BOOST_LOG(error) << "Windows Graphics Capture activation factory is unavailable";
      return -1;
    }

    if (auto target = palpeek::capture_target()) {
      status = monitor_factory->CreateForWindow(
        target->hwnd,
      ...
      if (FAILED(status)) {
        palpeek::set_capture_state(
          ...
          "Windows Graphics Capture could not open the selected game window"
        );
        BOOST_LOG(error) << "PalPeek could not capture the selected game window [0x"
                         << util::hex(status).to_string_view() << ']';
        return -1;
      }

      palpeek_target_generation = target->generation;
      const auto size = item.Size();
      display->offset_x = 0;
      display->offset_y = 0;
      display->width = size.Width;
      display->height = size.Height;
      display->width_before_rotation = size.Width;
      display->height_before_rotation = size.Height;
      display->display_rotation = DXGI_MODE_ROTATION_IDENTITY;
      BOOST_LOG(info) << "PalPeek capturing game window [" << size.Width << 'x' << size.Height << ']';
    } else {
      palpeek::set_capture_state(
        palpeek::capture_state_t::error,
        "target_unavailable",
        "No authorized game window is available for capture"
      );
      BOOST_LOG(error) << "PalPeek refused capture because no game window target is set";
      return -1;
    }
    ...
    try {
      ...
      palpeek::set_capture_state(
        ...
        "Windows Graphics Capture could not create a session for the selected game window"
      );
    ...
    try {
      ...
      palpeek::set_capture_state(
        ...
        "Windows Graphics Capture could not start the selected game window source"
      );
    ...
    palpeek::set_capture_state(palpeek::capture_state_t::capturing);
    return 0;
  }
  ...
  void wgc_capture_t::on_frame_arrived(winrt::Direct3D11CaptureFramePool const &sender, winrt::IInspectable const &) {
    ...
    try {
      ...
      palpeek::set_capture_state(
        ...
        "Windows Graphics Capture failed while reading the selected game window"
      );
  ...
  capture_e wgc_capture_t::next_frame(std::chrono::milliseconds timeout, ID3D11Texture2D **out, uint64_t &out_time) {
    if (palpeek_target_generation != 0) {
      auto target = palpeek::capture_target();
      if (!target) {
        palpeek::set_capture_state(
          palpeek::capture_state_t::error,
          "window_unavailable",
      ...
      if (target->generation != palpeek_target_generation) {
